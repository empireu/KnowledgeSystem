...
public sealed class Test
{
    ...
    private readonly IntegerArgument _topCountArgument;
    ...
    public Test(ILogger<Test> logger, RagEngine engine, Description description, string query)
    {
        ...
        _semanticSearchTool = new ToolBuilder("semantic_search")
            ...
            .WithRequiredStringArgument("query", "The search query. Use '|' to separate multiple queries.", out _queryArgument)
            .WithIntegerArgument("maxCount", "The maximum number of results per query. Defaults to 10.", out _topCountArgument)
            .Build();
    ...
    private async Task SemanticSearch(ArgumentExtractionResult args)
    { 
        var query = _queryArgument.GetValue(args);
        var specifiedTopK = false;
        
        if (!_topCountArgument.TryGetValue(args, out var topK))
        {
            topK = 5;
        }
        else
        ...
        if (specifiedTopK)
        {
            _logger.LogInformation("Tool: semantic_search(\"{q}\", {k})", query, topK);
        }
        else
        {
            _logger.LogInformation("Tool: semantic_search(\"{q}\")", query);
        }

        var resultBatches = await _engine.SearchAsync(query.Split('|'), topK, excludedIndices: ExcludedIndices);

        var vectorSearchResults = new List<VectorSearchResult>();
        var vectorSearchResultsSet = new HashSet<VectorSearchResult>();
        for (var batchIndex = 0; batchIndex < resultBatches.Length; batchIndex++)
        {
            var batch = resultBatches[batchIndex];
            
            for (var resultIndex = 0; resultIndex < batch.Length; resultIndex++)
            {
                var result = batch[resultIndex];
                if (vectorSearchResultsSet.Add(result))
                {
        ...
        // Chunks strictly returned by the query:
        var queryRootChunks = new List<EmdChunk>();
        
        // Total char count of raw query results and all their dependencies:
        var totalCharsWithDependencies = 0;
       
        // If true, the results were cut off due to length:
        var truncated = false;
      
        for (var searchResultIndex = 0; searchResultIndex < vectorSearchResults.Count; searchResultIndex++)
        {
            var vectorSearchResult = vectorSearchResults[searchResultIndex];
            ...
            queryRootChunks.Add(chunk);

            totalCharsWithDependencies += chunk.RawContent.Length; 
            
            var chunkDependencies = _engine.ResolveDependencies([chunk]);
            for (var dependencyChunkIndex = 0; dependencyChunkIndex < chunkDependencies.Count; dependencyChunkIndex++)
            {
                totalCharsWithDependencies += chunkDependencies[dependencyChunkIndex].RawContent.Length;
            }
            
            if (totalCharsWithDependencies > 10000)
            {
                truncated = searchResultIndex != vectorSearchResults.Count - 1;
                break;
            }
        }
        
        var dependencyChunks = _engine.ResolveDependencies(queryRootChunks);

        // Add dependency chunks to excluded set so they won't appear in future searches
        for (var dependencyChunkIndex = 0; dependencyChunkIndex < dependencyChunks.Count; dependencyChunkIndex++)
        {
            ExcludedIndices.Add(_engine.GetHnswId(dependencyChunks[dependencyChunkIndex]));
        }
        ...
        if (truncated)
        {
            sb.AppendLine("Warning: some results were dropped due to size!\nSemantic search results:\n");
        }

        FormatSemanticSearch(sb, queryRootChunks, dependencyChunks);
        
        _logger.LogInformation("Final result size: {size}, across {r} root results, with {d} dependencies", sb.Length, queryRootChunks.Count, dependencyChunks.Count);
